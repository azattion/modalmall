...
@section('content')
    <div class="row">
        ...
        <div class="col-sm-10">
            ...
            @if(count($products))
                @if($category['id'] != 83)
                    <form action="{{route('products.category', isset($category['id'])?$category['id']:0)}}"
                          method="get">
                        <div class="row">
                            @if($category)
                                <div class="col-sm text-left">
                                    <label for="sort">Сортировать по:
                                        <select id="sort">
                                            ...
                                            <option @if(isset($_GET['sort']) && $_GET['sort'] == 'top' && isset($_GET['order']) && $_GET['order'] == "asc") selected
                                                    @endif value="{{route('products.category', $category['id'])}}?sort=top&order=asc">
                                                Популярные
                                            </option>
                                            <option @if(isset($_GET['sort']) && $_GET['sort'] == 'cost' && isset($_GET['order']) && $_GET['order'] == "asc") selected
                                                    @endif value="{{route('products.category', $category['id'])}}?sort=cost&order=asc">
                                                По цене по возрастанию
                                            </option>
                                            <option @if(isset($_GET['sort']) && $_GET['sort'] == 'cost' && isset($_GET['order']) && $_GET['order'] == "desc") selected
                                                    @endif value="{{route('products.category', $category['id'])}}?sort=cost&order=desc">
                                                По цене по убыванию
                                            </option>
                                        </select>
                                    </label>
                            ...
                            <div class="col-sm">
                                <button type="submit" class="btn btn-link" style="padding: 17px 0 0">
                ...
                <div class="row product-row">
                    @foreach($products as $product)
                        <div class="col-md-{{$category['id']==83?"6":"4"}}">
                            @include('layouts.product-card', ['product' => $product])
                        </div>
                    @endforeach
                </div>
                <div class="no-margin float-right">
                    {{ $products->links() }}
                </div>
            @else
                <p>Каталог пока пустой (</p>
            @endif
        </div>
    </div>
@endsection
