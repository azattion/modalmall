...
<head>
    ...
    <style>
        ...
        .product__img {
            ...
            /*height: 280px;*/
            background: url(/img/frame-middle.png) top/100% no-repeat;
        }
...
<style>
    ...
    .nav-control:checked ~ .mob-nav ul li {
        opacity: 1;
    ...
    .toggle-button .wolverine.close .claws:before, .toggle-button .wolverine.close .claws:after {
        background: #ee6688;
    ...
    <ul>
        ...
        @foreach($cats[0] as $item)
            <li class="mob-sub active">
                ...
                @if(isset($cats[$item['id']]))
                    <ul>
                        @foreach($cats[$item['id']] as $item2)
                            <li>
                                <a href="{{route('products.category', $item2['id'])}}">{{$item2['name']}}</a>
                                @if(isset($cats[$item2['id']]))
                                    <ul>
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-3 text-center">
                    <a href="{{route('home')}}"><img style="width: 245px" src="/img/logo.png" alt="Logo"></a>
                </div>
                    ...
                    <ul class="main-nav list-unstyled row" style="margin-bottom: 0">
                        ...
                        @foreach($cats[0] as $item)
                            <li class="main-nav__item col sub">
                                ...
                                @if(isset($cats[$item['id']]))
                                    <ul class="list-unstyled main-nav__list text-left row">
                                        @foreach($cats[$item['id']] as $item2)
                                            <li class="main-nav__item col">
                                                <a class="main-nav__link main-nav__header"
                                                   href="{{route('products.category', $item2['id'])}}">{{$item2['name']}}</a>
                                                @if(isset($cats[$item2['id']]))
                                                    <ul class="list-unstyled text-left">
                                                        @foreach($cats[$item2['id']] as $item3)
                                                            <li class="main-nav__item">
                                                                <a class="main-nav__link"
                                                                   href="{{route('products.category', $item3['id'])}}">{{$item3['name']}}</a>
                                                        @endforeach
                                                    </ul>
                                                @endif
                                            </li>
                    ...
                    <div class="footer-soc-btn">
                        ...
                        <div class="row">
                            ...
                            <a class="footer-soc-btn__link col" href="https://t.me/Modalmall">
                                <img class="footer-soc-btn__icon" src="data:image/svg+xml;base64,
"/>
                            ...
                            <a class="footer-soc-btn__link col" href="https://api.whatsapp.com/send?phone=79898346581">
                                <img class="footer-soc-btn__icon" src="data:image/svg+xml;base64,
"/></a>
                            <a class="footer-soc-btn__link col" href="https://www.instagram.com/buonumare_krd/">
                                <img class="footer-soc-btn__icon"
                            ...
                            <a class="footer-soc-btn__link col" href="#">
                                <img class="footer-soc-btn__icon"
                            ...
                            <a class="footer-soc-btn__link col" href="mailto:modalmall@bk.ru">
                                <img class="footer-soc-btn__icon"
